...
class fun(commands.Cog):
    ...
    async def agent(self, ctx):
        ...
        embed = nextcord.Embed(title="Agent Select | Moon Bot",
                               description=f"{ctx.author.mention} Your agent is: **{random.choice(responses)}**")
    ...
    async def gunselect(self, ctx):
        ...
        embed = nextcord.Embed(
            title="Gun Selection", description=f"{ctx.author.mention} Your gun is: **{random.choice(responses)}**")
        await ctx.send(embed=embed)
    ...
    async def coinflip(self, ctx):
        ...
        if random.choice(determine_flip) == 1:
            embed = nextcord.Embed(
                title="Coinflip", description=f"{ctx.author.mention} Flipped coin, we got **Heads**!")
            await ctx.send(embed=embed)

        else:
            embed = nextcord.Embed(
                title="Coinflip", description=f"{ctx.author.mention} Flipped coin, we got **Tails**!")
            await ctx.send(embed=embed)
    ...
    #                 member.guild.text_channels, id=config.BOT_COMMAND_CHANNEL_ID)
    ...
    #             '/home/minsung/DiscordBot/soundTracks/firstOne.mp3')
    #         player = voiceplay.play(source)
